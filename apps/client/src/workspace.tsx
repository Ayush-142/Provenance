...
import type { EventType, RunResult, TestCase } from "@provenance/shared";
...
export function Workspace(): JSX.Element {
  ...
  const onEditorMount: OnMount = (editor) => {
    editor.onDidPaste((event) => {
      ...
      if (pasted.length > 40) enqueue("CODE_PASTED", { content: pasted, matchesRecentAi: false });
    });
  };
    ...
    try {
      ...
      while (true) {
        ...
        for (const messageEvent of messages) {
          ...
          if (event.type === "done") setChat((items) => [...items.slice(0, -1), { role: "tutor", text: answer, category: event.category }]);
  ...
  return <div className="flex h-screen flex-col overflow-hidden">
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6">
      ...
      <div className="flex items-center gap-5 text-sm"><span className="font-mono text-slate-500">SESSION {formatDuration(elapsed)}</span><span className="h-2 w-2 rounded-full bg-emerald-500" /> <span className="text-slate-600">Recording process</span></div>
    </header>
